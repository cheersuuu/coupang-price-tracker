...
# ── 요약 지표
latest_df = df[df["Date"].dt.date == latest].copy()
...
if selected:
    filtered = df[df["SKU"].isin(selected)]
    fig = px.line(
        filtered, x="Date", y="가격", color="SKU",
...
show_df = latest_df[["SKU","개수","수량","가격"]].copy()
show_df = show_df.sort_values("가격", ascending=False)
st.dataframe(
